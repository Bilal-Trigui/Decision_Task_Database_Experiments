...
Real runs: open `run_experiment.ipynb` in Colab, set `USE_TEST_CONFIG = False` and
`SETTINGS_FILE` in the first cell, and run all cells. The setup cell clones this
branch, runs `run_me.py`, and installs `requirements.txt`. Set `HF_TOKEN` in the
Colab secrets or environment first; the backend prints
`LOGIN CREDENTIALS NEEDED HERE: HF_TOKEN` and stops if it is missing.
...
them from the seeds otherwise, builds introspection-training examples from
`introspection_training.csv` or generates and saves that file, and draws the fresh
...
**`data/plunkett/`** is a copy of Plunkett's `data/` folder, file names and columns
unchanged. **`data/a3/`** was built from it by the interaction rule.
